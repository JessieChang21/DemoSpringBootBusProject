...
<body>
	...
	<BR />
	<div id="left">
		<span id="tripname">查詢起站 : ${inputdataObj.initstation}</span>
		<span id="tripname">查詢迄站 : ${inputdataObj.finalstation}</span>
		...
		<c:otherwise>
			<div id="table_right">
				<form action="/members/bookTicket.controller" method="POST">
					<table id="showbus">
						<tr>
							<th colspan="11" id="showdirection"></th>
						</tr>
						<tr>
							<th>車次</th>
							<th>路線名稱</th>
					...
					<input id="direction" value="${resultdirection}" type="hidden">
				</form>
			</div>
		</c:otherwise>
	</c:choose>
	...
	<script type="text/javascript">
		...
		window.onload = function() {
			...
			if($("#direction").val() == 0){
				$("#showdirection").html("順向");
			}else{
				$("#showdirection").html("逆向");
			}
